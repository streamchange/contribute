...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    if (user.email.endsWith('@scenechange.com.au')){
      console.log('Welcome Friend')
    } else {
      auth.signOut();
      alert("Sorry, you must sign in using an approved email domain, please contact your system administrator if you believe this to be an error")

      //location.reload();
    }
    whenSignedIn.hidden = false;
    signup.hidden = true;
    userDetails.innerHTML = `<img src="${user.photoURL}"><br><h3>Hello👋, ${user.displayName}</h3><p>${user.email}<br><br><button type="button" id="signOutBtn" onclick="auth.signOut();
